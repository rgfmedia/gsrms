			...
			<div class="menu_nav">
				<ul>
				    <ul>
										...
										<li class="active"><a href="minute2.php">Minutes</a></li>
          ...
          <thead>
           <tr bgcolor="0c2883" style="margin-bottom:10px;">
		   		            	...
		   		            	<?php if ($_SESSION['SESS_FIRST_NAME']=="admin"){
		   		  					  echo '<a rel="facebox" href="addminute2.php"><input name="button" type="button" style="cursor:pointer;" value="ADD FILES"/></a>';
		   		  					  }
		   		  					  ?>
		   		  			</th>
				...
				while($row = mysql_fetch_array($result))
				  {
					echo '<tr>';
					  echo '<td>'.$row['date'].'</td>';
					  echo '<td>'.$row['title'].'</td>';
					  echo '<td><div align="center">'.$row['officer'].'</div></td>';
					   if ($_SESSION['SESS_FIRST_NAME']=="admin"){
					  echo '<td><div align="center">'.'<a href=download.php?id=' . $row["content"] .'&amp;downloadby=argie>download</a>'.' | '.'<a rel="facebox" href=editmemo.php?id=' . $row["min_id"] .'><img src="images/b_edit.png" /></a>'.' </div></td>';
					echo '</tr>';
						}
					  if ($_SESSION['SESS_FIRST_NAME']!="admin"){
